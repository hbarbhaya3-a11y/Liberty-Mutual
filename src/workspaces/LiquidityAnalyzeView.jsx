/* ============================================================================
   LiquidityAnalyzeView — Hypothesis Detail for Idle-Cash Liquidity Activation.

   Forked from RetentionAnalyzeView.jsx structure. Same chapters, same JSX
   class conventions (so analyse-workspace.css styles apply unchanged),
   different content + liquidity-specific mini-charts.

   Recommended hypothesis: H-LIQ-2026-03-12 (Idle-to-Yield CD).

   Sections (single scroll):
     1. Hero                         — recommended hypothesis + pre-sim KPI ranges
     2. Chapter 01 · Data + models   — 3 collapsible asset groups from liquiditySensedAssets
     3. Chapter 02 · Drivers         — 5 reason cards with liquidity mini-charts
...
import liquiditySensedAssets from "@/data/liquiditySensedAssets";
import {
  LIQUIDITY_REASON_CARDS,
  JOINT_SIGNAL_BINS,
  VOLATILITY_BARS,
  PRIMACY_DECAY,
  STICKINESS_DISTRIBUTION,
  PRODUCT_DEPTH,
} from "@/data/liquidityCohortInsights";
import {
  LIQUIDITY_HYPOTHESIS_ID,
  LIQUIDITY_PRESIM_RANGES,
  liquidityHypothesis,
...
/* ----------------------------------------------------------------------------
   ...
   Vertical bar histogram of idle-balance $ bands. $20-50K and $50-100K are
  ...
  return (
    <div className="mini-chart">
            ...
            <div className="mini-bar-h-track">
              <div
                ...
                onMouseEnter={() => setHover({
                  label: r.band,
                  value: r.target
                    ? `${r.freq}% of cohort — actionable surplus idle-cash target band`
      ...
      <div className="mini-chart-foot">
        Deep idle balances concentrate at <b>$20–100K</b> — the surplus pool the activation strategy targets.
      </div>
      {hover && (
...
/* ----------------------------------------------------------------------------
   ...
   Horizontal bars by held-vs-market yield-gap band. Bands 3.0-5.0pp are
   actionable.
   -------------------------------------------------------------------------- */
function VolatilityBars() {
  ...
  return (
    <div className="mini-chart">
            ...
            <div className="mini-bar-h-track">
              <div
                ...
                onMouseEnter={() => setHover({
                  label: r.ratio,
                  value: r.target
                    ? `${r.pct}% of cohort — yield gap wide enough to motivate activation`
                    : `${r.pct}% — outside the actionable yield-gap window`,
                })}
                onMouseLeave={() => setHover(null)}
              />
            </div>
      ...
      <div className="mini-chart-foot">
        59% of the cohort runs a <b>3.0–5.0pp</b> yield gap — wide enough to make idle cash actively expensive to hold.
      </div>
      {hover && (
        <div className="mini-chart-tip">
          <span className="mini-chart-tip-l">{hover.label}</span>
...
/* ----------------------------------------------------------------------------
   ...
   Per-archetype paired bars: search-intent + outbound-pull index.
   Higher value = further along the idle-flight progression.
   -------------------------------------------------------------------------- */
function PrimacyDecayBars() {
  ...
  return (
    <div className="mini-chart">
          ...
          <div className="mini-bar-h-row" key={i}>
            ...
            <div className="mini-bar-h-track" style={{ display: "flex", gap: "3px" }}>
              <div
                ...
                style={{ width: `${r.dd * 50}%` }}
                onMouseEnter={() => setHover({ label: `${r.archetype} · search intent`, value: `${(r.dd*100).toFixed(0)}% high-yield search-intent index` })}
                onMouseLeave={() => setHover(null)}
              />
              <div
                className={`mini-bar-h-fill${r.bp > 0.6 ? " bad" : r.bp > 0.4 ? " warn" : " ok"}`}
                style={{ width: `${r.bp * 50}%` }}
                onMouseEnter={() => setHover({ label: `${r.archetype} · outbound pull`, value: `${(r.bp*100).toFixed(0)}% outbound-pull index` })}
                onMouseLeave={() => setHover(null)}
              />
            </div>
      ...
      <div className="mini-chart-foot">
        Bonus Holder shows the strongest joint search + outbound signal — the activation target before the balance leaves.
      </div>
      {hover && (
        <div className="mini-chart-tip">
          <span className="mini-chart-tip-l">{hover.label}</span>
...
/* ----------------------------------------------------------------------------
   ...
   Histogram by suitability-discriminator score. Bins below 0.55 = operating /
   emergency buffers (NOT eligible). Bins above = activatable surplus idle cash.
   -------------------------------------------------------------------------- */
function StickinessDistribution() {
  ...
  return (
    <div className="mini-chart">
            ...
            <div className="mini-bar-h-track">
              <div
                ...
                onMouseEnter={() => setHover({
                  label: r.bin,
                  value: r.sticky
                    ? `${r.pct}% — operating / emergency buffer · NOT offered the term product (must stay liquid)`
                    : `${r.pct}% — activatable surplus idle cash · eligible for the offer`,
                })}
                onMouseLeave={() => setHover(null)}
              />
            </div>
      ...
      <div className="mini-chart-foot">
        Gate at <b>0.55</b> — 34% of cohort scores below it (operating / emergency buffers, held out of the offer).
      </div>
      {hover && (
        <div className="mini-chart-tip">
          <span className="mini-chart-tip-l">{hover.label}</span>
...
/* ----------------------------------------------------------------------------
   ...
   cohort, the largest single-product idle sub-segment to activate.
   -------------------------------------------------------------------------- */
function ProductDepthBars() {
  ...
  return (
    <div className="mini-chart">
            ...
            <div className="mini-bar-h-track">
              <div
                ...
                onMouseEnter={() => setHover({
                  ...
                  value: r.amplified
                    ? `${r.pct}% — single-product idle holders · largest activatable sub-segment`
      ...
      <div className="mini-chart-foot">
        Single-product idle holders are the largest sub-segment to activate — <b>49%</b> of the cohort.
      </div>
      {hover && (
        <div className="mini-chart-tip">
          <span className="mini-chart-tip-l">{hover.label}</span>
...
/* ----------------------------------------------------------------------------
   Chart dispatcher — keyed by LIQUIDITY_REASON_CARDS[i].chart field
   -------------------------------------------------------------------------- */
...
/* ============================================================================
   LiquidityAnalyzeView
   ========================================================================= */
export default function LiquidityAnalyzeView() {
  const {
  ...
  useEffect(() => {
    if (!selectedThemeId) selectTheme("liquidity");
    if (!selectedHypothesisId) selectHypothesis(LIQUIDITY_HYPOTHESIS_ID);
  ...
  const assetsByGroup = (g) => liquiditySensedAssets.filter((a) => a.group === g);

  const [testMenuOpen, setTestMenuOpen] = useState(false);
  // The hypothesis to detail is whichever the Theme page's signal card selected
  // (selectedHypothesisId), falling back to the recommended. The 3 cards live on
  // the Theme page — this is the detail page for the one the user picked.
  const detailHyp = liquidityHypothesis(selectedHypothesisId || LIQUIDITY_HYPOTHESIS_ID);
  const goTest = (hypId = detailHyp.id) => {
    selectHypothesis(hypId);
    setTestMenuOpen(true);
  };
  const goBackToTheme = () => routerNavigate("/theme?id=liquidity");
      ...
      <div className="wrap">
        ...
        <header className="aw-hero">
          ...
          <div className="aw-hero-kpis">
            {LIQUIDITY_PRESIM_RANGES.map((r, i) => (
        ...
        <section className="aw-chapter" id="aw-chapter-data">
          <div className="aw-chapter-h">
            ...
            <div className="aw-chapter-meta-row">
              <span className="aw-chapter-meta">{liquiditySensedAssets.length} composed</span>
        ...
        <section className="aw-chapter">
          ...
          <div className="reason-list">
            {LIQUIDITY_REASON_CARDS.map((r, i) => (
