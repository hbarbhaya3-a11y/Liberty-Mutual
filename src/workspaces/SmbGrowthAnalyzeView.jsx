/* ============================================================================
   SmbGrowthAnalyzeView — Hypothesis Detail for SMB Growth / Re-bundle.

   Forked from SmbRateAnalyzeView.jsx structure. Same hero, same chapters,
   ...
   apply unchanged). Different content + growth-specific mini-charts.

   Recommended hypothesis: H-SMB-GROWTH-2026-03-04 (Re-bundle the off-us flows).

   Sections (single scroll):
     1. Hero                         — recommended hypothesis + pre-sim KPI ranges
     2. Signals                      — 3 strategy signal cards (SMBGROWTH_STRATEGIES)
     3. Chapter 01 · Data + models   — compact data-stream + model groups
     4. Chapter 02 · Drivers         — reason cards with trigger-bar mini-charts
                                       + a brief current-holdings white-space strip
...
import {
  ...
  SMBGROWTH_CONFIG,
  SMBGROWTH_PRESIM_RANGES,
  SMBGROWTH_STRATEGIES,
  SMBGROWTH_DRIVERS,
  SMBGROWTH_HOLDINGS,
...
/* ----------------------------------------------------------------------------
   ...
   Horizontal bars of the SMB count per expansion trigger. Each bar is sized by
   its count relative to the largest driver. Reuses the mini-bars-h markup.
   -------------------------------------------------------------------------- */
  ...
  return (
    <div className="mini-chart">
      ...
      <div className="mini-chart-foot">
        <b>{fmt(driver.pct)} {driver.unit}</b> showing this trigger across the expansion cohort.
      </div>
      {hover && (
...
/* ============================================================================
   SmbGrowthAnalyzeView
   ========================================================================= */
export default function SmbGrowthAnalyzeView() {
  const {
  ...
  // upstream selection (mirrors the rate view's behaviour).
  useEffect(() => {
    if (!selectedThemeId) selectTheme("smbgrowth");
    if (!selectedHypothesisId) selectHypothesis(SMBGROWTH_HYPOTHESIS_ID);
  ...
  const goTest = () => {
    selectHypothesis(SMBGROWTH_HYPOTHESIS_ID);
    setTestMenuOpen(true);
  };
  const goBackToTheme = () => routerNavigate("/theme?id=smbgrowth");
  ...
  const openSignalInLoop = (hid) => {
    selectHypothesis(hid || SMBGROWTH_HYPOTHESIS_ID);
    setTestMenuOpen(true);
  };

  const accent = SMBGROWTH_CONFIG.accent;
  const maxDriver = Math.max(...SMBGROWTH_DRIVERS.map((d) => d.pct));
  const maxHolding = Math.max(...SMBGROWTH_HOLDINGS.map((h) => h.pct));
  ...
  return (
    <div className="aw theme-page" style={{ "--acc": accent, "--acc-soft": accent + "22" }}>
      <div className="wrap">
        ...
        <header className="aw-hero">
          <div className="aw-hero-tag">
            <span className="aw-hero-star"><Icon name="star" size={12} /></span> Recommended hypothesis · {SMBGROWTH_HYPOTHESIS_ID}
          </div>
          <h1 className="aw-hero-title">{SMBGROWTH_HYPOTHESIS_TITLE}</h1>
          <p className="aw-hero-desc">{SMBGROWTH_CONFIG.claim}</p>
          ...
          <div className="aw-hero-kpis">
            {SMBGROWTH_PRESIM_RANGES.map((r, i) => (
        ...
        <section className="aw-chapter">
          <div className="aw-chapter-h">
            ...
            <div className="aw-chapter-meta">{SMBGROWTH_DRIVERS.length} weighted</div>
          </div>

          <div className="reason-list">
            {SMBGROWTH_DRIVERS.map((r, i) => (
              ...
              <div className="mini-bars-h">
                {SMBGROWTH_HOLDINGS.map((h, i) => (
                  <div className="mini-bar-h-row" key={i}>
                    ...
                    <div className="mini-bar-h-track">
                      <div
                        className="mini-bar-h-fill"
                        style={{ width: `${(h.pct / maxHolding) * 100}%` }}
                      />
                    </div>
      ...
      <TestModeChooser open={testMenuOpen} onClose={() => setTestMenuOpen(false)} eyebrow={"TESTING · " + SMBGROWTH_HYPOTHESIS_TITLE.toUpperCase()} />
    </div>
  );
}
